...
function pipeOkApi(overrides: Partial<Win32ProcessBindings> = {}): {
  ...
  const api = {
    ...
    createProcessAsUserW: vi.fn((
      ...
      koffi.encode(processInfo, processInfoStruct(), { hProcess: 200n, hThread: 201n, dwProcessId: 1234, dwThreadId: 5678 })
      return 1
    }),
  ...
  it('rejects NULL process/thread handles after a successful spawn', () => {
    const { api } = pipeOkApi({
      createProcessAsUserW: vi.fn((
        ...
        koffi.encode(processInfo, processInfoStruct(), { hProcess: null, hThread: null, dwProcessId: 1234, dwThreadId: 5678 })
        return 1
      }),
    })
